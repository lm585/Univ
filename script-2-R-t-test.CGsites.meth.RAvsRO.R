...
cat temp-ENSG-files genes-constant-meth-t-test-error.RAvRO.genes.txt  | sort | uniq -u > temp-441pm
mv temp-441pm temp-ENSG-files

cat temp-ENSG-files |  while read file
do
 echo "name <- c(\"$file\")";
 cut -f 2 $file > temp-s1
 cut -f 6 $file >> temp-s1
 cut -f 3 $file > temp-s2
 nl=`wc -l temp-s2 | awk '{print $1}'`
 cut -f 7 $file >> temp-s2
 echo "nl = $nl"
